...
Раздел `График` в ЛК уже умеет читать месяц из Postgres, импортировать JSON-бэкап текущего календаря и редактировать календарь через защищенный API для руководителя: смены, дедлайны, плановые дни зарплаты, фактические выплаты и точки оценки.
