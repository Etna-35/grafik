...
Раздел `График` в ЛК уже умеет читать месяц из Postgres, импортировать JSON-бэкап текущего календаря и редактировать календарь через защищенный API для руководителя: смены, дедлайны, плановые дни зарплаты, фактические выплаты и точки оценки. Визуально ЛК ориентируется на старый график: компактная цветная сетка, колонка `Σ`, ФОТ/план и карточки итогов. Для обычного сотрудника график открывается в безопасном режиме: общий график и точки оценок видны, но чужие деньги, ФОТ, план выручки и ставки скрыты. Внутренний раздел `/admin` в ЛК позволяет руководителю заводить сотрудников, менять PIN, параметры графика и индивидуальные доступы к сервисам.
