...
async function computeState() {
  ...
  // --- ФОТ как датированное обязательство: ЗП за месяц гасится к 10-му СЛЕДУЮЩЕГО месяца ---
  // В текущем месяце платится ЗП за ПРОШЛЫЙ месяц, поэтому ФОТ НЕ вычитаем из дневного FCF,
  // а показываем обязательством. owed(месяц) = начислено по графику − выплачено за этот месяц (payroll).
  const [yy0, mm0] = today.split("-").map(Number);
  const salStart = isoOf(new Date(Date.UTC(yy0, mm0 - 1 - 2, 1)));
  const [accrRows, paidRows] = await Promise.all([
    query<{ ym: string; v: string }>(
      `SELECT to_char(work_date, 'YYYY-MM') AS ym, COALESCE(SUM(pay_amount), 0)::text AS v
       FROM schedule_shifts WHERE work_date >= $1::date GROUP BY 1`,
      [salStart]
    ),
    query<{ ym: string; v: string }>(
      `SELECT to_char(COALESCE(apply_month, date_trunc('month', work_date)::date), 'YYYY-MM') AS ym,
              COALESCE(SUM(amount), 0)::text AS v
       FROM payroll_payouts GROUP BY 1`
    )
  ]);
  const accrByM = new Map<string, number>();
  for (const r of accrRows.rows) accrByM.set(r.ym, Number(r.v || 0));
  const paidByM = new Map<string, number>();
  for (const r of paidRows.rows) paidByM.set(r.ym, Number(r.v || 0));
  const monthKey = (offset: number) => {
    const d = new Date(Date.UTC(yy0, mm0 - 1 + offset, 1));
    return `${d.getUTCFullYear()}-${String(d.getUTCMonth() + 1).padStart(2, "0")}`;
  };
  const owedFor = (ymKey: string) => Math.max(0, Math.round((accrByM.get(ymKey) || 0) - (paidByM.get(ymKey) || 0)));
  const tenthOfNext = (ymKey: string) => {
    const [y, m] = ymKey.split("-").map(Number);
    return isoOf(new Date(Date.UTC(y, m, 10)));
  };
  const RU_MONTHS = ["январь", "февраль", "март", "апрель", "май", "июнь", "июль", "август", "сентябрь", "октябрь", "ноябрь", "декабрь"];
  const monthLabel = (ymKey: string) => RU_MONTHS[Number(ymKey.split("-")[1]) - 1] || ymKey;

  type Ob = { id: string; title: string; amount: number; dueDate: string; category: string; priority: number; splittable: boolean; recurring: string; isSalary: boolean };
  const salaryObs: Ob[] = [];
  for (const off of [-2, -1, 0]) {
    const mk = monthKey(off);
    const owed = owedFor(mk);
    if (owed > 0) {
      salaryObs.push({ id: `salary-${mk}`, title: `ЗП за ${monthLabel(mk)}`, amount: owed, dueDate: tenthOfNext(mk), category: "ЗП", priority: 0, splittable: false, recurring: "none", isSalary: true });
    }
  }

  // Объединённый список обязательств (ЗП + платежи) для единого расчёта покрытия.
  const obs: Ob[] = [
    ...salaryObs,
    ...payRows.rows.map((p) => ({ id: p.id, title: p.title, amount: Number(p.amount), dueDate: p.due_date, category: p.category, priority: p.priority, splittable: p.splittable, recurring: p.recurring, isSalary: false }))
  ].sort((a, b) => (a.dueDate < b.dueDate ? -1 : a.dueDate > b.dueDate ? 1 : a.priority - b.priority));

  // Прогноз дневного свободного потока (FCF). ФОТ НЕ вычитаем — он отдельное обязательство.
  const opPct = (settings.purchasePct + settings.householdPct) / 100;
  const wavg = await weekdayAvg(today);
  const maxDue = obs.reduce((acc, p) => (p.dueDate > acc ? p.dueDate : acc), today);
  ...
  for (let i = 1; i <= horizonDays; i++) {
    ...
    const fcf = Math.round(rev - rev * opPct);
    cum += fcf;
  ...
  const coveredNowList = obs.map((p) => {
    const c = Math.max(0, Math.min(p.amount, poolNow));
    poolNow -= c;
    return c;
  });
  ...
  let consumed = 0;
  let todayEarmark = 0;
  const computed = obs.map((p, i) => {
    const amount = p.amount;
    const daysLeft = Math.max(1, daysBetween(today, p.dueDate));
    const consumedBefore = consumed;
    const availableByDue = reservableNow + cumUpTo(p.dueDate) - consumedBefore;
    ...
    // Подсказки при нехватке (только для пользовательских платежей; ЗП не двигаем/не делим).
    ...
    if (!p.isSalary && statusFlag !== "ok") {
      const feasible = fcfSeries.find((s) => s.iso > p.dueDate && reservableNow + s.cum - consumedBefore >= amount);
      const moveToDate = feasible ? feasible.iso : null;
      const reduce = obs
        .filter((q, j) => j !== i && q.dueDate <= p.dueDate)
        .slice(0, 3)
        .map((q) => ({ title: q.title, amount: q.amount }));
      suggestions = { moveToDate, splittable: p.splittable, splitNow: coverage, splitLater: shortfall, splitLaterDate: moveToDate, reduce };
    }
  ...
  const salary = computed
    .filter((c) => c.isSalary)
    .map((c) => ({ id: c.id, title: c.title, amount: c.amount, dueDate: c.dueDate, coverage: c.coverage, shortfall: c.shortfall, pctCovered: c.pctCovered, statusFlag: c.statusFlag, perDay: c.perDay }));
  const payments = computed.filter((c) => !c.isSalary);

  // Свободно сегодня = дневной свободный поток − рекомендованные отчисления (ФОТ не вычитаем из FCF).
  const todayDow = new Date(`${today}T00:00:00Z`).getUTCDay();
  const todayRev = todayRevenue > 0 ? todayRevenue : Math.round(wavg.get(todayDow) || 0);
  const todayFcf = Math.round(todayRev - todayRev * opPct);
  const freeToday = Math.round(todayFcf - todayEarmark);

  // Распределение прогноза месяца: ЗП к выплате в этом месяце = ФОТ ПРОШЛОГО месяца (за вычетом выплат).
  const [yy, mm] = today.split("-").map(Number);
  const ym = `${yy}-${String(mm).padStart(2, "0")}`;
  const monthRev = (await predictRevenue(yy, mm)).predicted;
  const salaryThisMonth = owedFor(monthKey(-1));
  ...
  const freeAlloc = Math.max(0, Math.round(monthRev - salaryThisMonth - purchaseAlloc - householdAlloc - reserveAlloc));
  const allocation = {
    revenue: monthRev,
    fot: salaryThisMonth,
  ...
  return { today, balance, balanceAsOf, settings, envelopes, salary, payments, freeToday, todayEarmark, allocation };
}
