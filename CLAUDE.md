...
- **Итоги недели** (`weeklyStats.ts`): мотивационно-развлекательный дайджест за ПРОШЕДШУЮ неделю (Пн–Вс, МСК).
  Метрики (топ-1, активные, без owner): Самый благодарный (больше всего отправленных «спасибо»), Любимчик недели
  (больше всего полученных), MVP (сумма очков прогресса за неделю), Знаток (тесты+главы), Мастер продаж (цели
  продаж + кэш-серии 036). Бэк: `GET /api/weekly-stats` (`getWeeklyDigest`), пустые метрики опускаются. ЛК:
  карточка «Итоги недели» на главной (renderWeeklyStats, монохром БЕЗ emoji, видна всю неделю; lazy-load
  `state.weeklyStats`). Telegram: `cron.ts` постит в общий чат **по понедельникам 11:12 МСК** (с emoji,
  `formatWeeklyDigestTelegram`, guard `lastWeeklyFired`).
