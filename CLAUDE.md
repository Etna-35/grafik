...
Обновлено: 2026-06-29. Ветка `main`, дерево чистое, посл. коммит `89a27c8`.
Блоки 1–4 + Финансы ✅ на проде (`docs/roadmap-blocks.md`). Свежие фичи сессии 2026-06 (миграции 038–043) —
см. §6/§7 ниже и `docs/HANDOFF.md` (шапка «🔴 СВЕЖЕЕ 2026-06-29»). Открытые хвосты — там же в HANDOFF.
...
039 employees.schedule_from/schedule_until (окно участия в графике по месяцам). Состав месяца в
`getScheduleMonth`: показываем ВСЕХ у кого есть смены в этом месяце (история, даже архивных) ∪ активных
не-архивных с schedule_role, у кого месяц >= `date_trunc('month', start_date)` И (schedule_until IS NULL ИЛИ
месяц <= `date_trunc('month', schedule_until)`). ВАЖНО: появление «с» завязано на **start_date** (дата начала
работы — она же стаж), отдельное поле schedule_from БОЛЬШЕ НЕ используется (Safari на Mac не поддерживает
`type=month` → поле ломалось). Управление: админ-форма — «Дата начала работы» (календарь) задаёт старт, кнопка
«Уволить» → `type=date` дата увольнения = schedule_until (скрывает со след. месяца); ИЛИ экран графика
(renderRosterEditor «Убрать из графика с <месяца>» = schedule_until месяц−1). Новичок: ставим дату начала
июлем → в июне не виден; уволенный: дата увольнения → июнь остаётся по сменам, июль скрыт ·
040 schedule_month_rates (индивидуальная ставка/час сотрудника НА МЕСЯЦ — год/месяц/employee_id PK, hourly_rate;
напр. су-шеф с июля дороже). `getScheduleMonth` отдаёт hourlyRate=override??штатная + baseRate + rateOverride;
`upsertShift` берёт ставку месяца при добавлении смены (если нет роли-дня и явной ставки). Эндпоинты (manager):
`PUT /api/schedule/month-rate` {year,month,employeeId,rate|null} — ставит/снимает + ПЕРЕСЧИТЫВАЕТ почасовые
смены ТОЛЬКО этого месяца (role_override IS NULL, pay_model='hourly'); `PUT /api/schedule/roster-window`
{employeeId,scheduleFrom?,scheduleUntil?} (обновляет только переданные поля). История прошлых месяцев цела
(деньги в schedule_shifts.pay_amount). UI: замок открыт → КЛИК ПО ИМЕНИ в шапке графика → renderRosterEditor
(ставка на месяц + «Убрать из графика с <месяца>»=schedule_until месяц−1). Фикс-должностям ставка не применяется ·
041 tasks.approved_at (приёмка личной задачи руководителем; см. §7 «Задачи» — премия/ТГ/очки только после approve) ·
042 schedule_shifts.day_part (пометка «утренняя/вечерняя» смена, NULL=весь день; ТОЛЬКО визуал, на расчёты не
влияет). Ставится в редакторе часов (селектор «Часть дня»), хранится в upsertShift, отдаётся как `dayPart`.
В ячейке — мягкий градиент сверху плашки (`.h.dp-morning` тёплый / `.h.dp-evening` прохладный), цвет должности
сохраняется, оценки/выплаты/ДР не задеваются (градиент = `::before`, не конфликтует с payout `::after`) ·
043 schedule_days.event_title/event_note (КОРПОРАТИВ/спецмероприятие). Смена-корпоратив: в редакторе часов
селектор «Тип смены»=Корпоратив → разовая выплата (`pay_model='event'`, не почасовая, индивид. сумма) + название
и заметка о мероприятии (хранятся на дне, общие). В ячейке — золотая плашка «К» (`.h.ev`), на дате — метка «К».
По клику на дату с меткой ЛЮБОЙ (в т.ч. сотрудник при закрытом замке) видит окно с инфо (renderEventPopup).
Выплата падает в доход как обычная смена (доход = SUM(pay_amount)). upsertShift: ветка `data.corporate`) ·
044 payroll_planned_days.amount (сумма планируемой выплаты в «дне зарплаты»; NULL = день отмечен, сумма не
задана). Рук-ль: окно даты → чекбокс «День зарплаты» → поле суммы + «Сохранить сумму» (`PUT
/api/schedule/planned-paydays` со `{amount}`, ON CONFLICT DO UPDATE). Сотрудник: тап по СВОЕЙ отмеченной
ячейке (замок закрыт) → окно-оверлей «День выплаты» с крупной суммой (renderPaydayPopup, стиль как у
корпоратива). Отдаётся в дне как `plannedPays:[{employeeId,amount}]` (фильтр тот же, что у
plannedPayEmployeeIds — сотрудник видит только своё). Если в этот же день есть своя почасовая смена —
в окне кнопка «Уточнить часы смены» (чтобы не потерять самоправку часов).
...
- **Задачи** (`tasks`): название/описание/дедлайн/**премия**(→Выплаты); статус-бейдж у названия; **ролевые задания**
  ...
  **Приёмка личной задачи (мигр. 041 `tasks.approved_at`):** сотрудник «Готово» → status='done' = «на проверке»
  (НИ премии в доходе, НИ ТГ, НИ очков). Рук-ль «Подтвердить» (`PATCH /api/tasks/:id/approve`, manager) →
  approved_at=now() + очки manager_task + ТГ «✅ выполнил личную задачу …» + премия учитывается. Возврат в
  работу (status='open') снимает approved_at и удаляет начисленные очки. Доход (schedule extras + payroll)
  гейтится `status='done' AND approved_at IS NOT NULL`, привязка к месяцу по approved_at. РОЛЕВЫЕ задачи
  засчитываются сразу при «Готово» (приёмки нет, премии нет). Бэкфилл миграции: старые done → approved_at=updated_at.
  Счётчик «На проверке» в метриках у рук-ля (teamSummary.awaiting).
...
- **seed ставит PIN владельца ОДИН РАЗ** (при создании учётки) и больше его не трогает — с 2026-07-30
  смена PIN в приложении переживает деплой. Владельца ищет ПО РОЛИ (не по имени — имена не уникальны).
  Аварийный сброс, если PIN забыт: в `/opt/etna/deploy/.env` задать нужный `OWNER_PIN` + `OWNER_PIN_RESET=1`,
  `docker compose up -d api`, затем **обязательно убрать `OWNER_PIN_RESET`** (иначе следующий деплой снова
  перезапишет PIN). В логах видно, что сделал seed: «PIN владельца не тронут» / «перезаписан».
