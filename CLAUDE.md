...
Фаза 1 ПОЛНОСТЬЮ на проде: бэк `treasury.ts` + синт. плитка «Касса» в getServices (только owner) + фронт-раздел
(renderTreasuryPage). В Финансах СПРЯТАНЫ P&L-таблица и «Постоянные платежи» (renderFinanceContent). Спец и
этапы 2-3 — `docs/treasury-planner.md`).
