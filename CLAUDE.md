...
(renderFinanceContent). Фаза 3 частично: авто-подсказки у платежей (перенести/сплит=эндпоинт
`/payments/:id/split`/урезать). Осталось: повтор. платежи, календарь месяца, история ставок, долги/профицит
— `docs/treasury-planner.md`).
